...
class ModularEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        ...
        alive_bonus = 1.0
        reward = reward - ctrl_cost - contact_cost + alive_bonus
        state = self.state_vector()
        done = not (np.isfinite(state).all() \
            and state[2] >= 0.2 and state[2] <= 1.0)
        ob = self._get_obs()
    ...
    def _get_obs(self):
        ...
        def _get_obs_per_limb(b):
            if b == 'torso':
                limb_type_vec = np.array((1, 0, 0))
            elif 'aux' in b:
                limb_type_vec = np.array((0, 1, 0))
            elif 'f' in b:
                limb_type_vec = np.array((0, 0, 1))
            else:
                limb_type_vec = np.array((0, 0, 0))
